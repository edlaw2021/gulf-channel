...
export async function getEventsForWeek(startDate, townId) {
  ...
  // pg returns DATE columns as JS Date objects, not strings — normalize to
  // 'YYYY-MM-DD' here so every component downstream can just call
  // e.date.slice(0, 10) without needing to know or care about that.
  return rows.map(row => ({
    ...row,
