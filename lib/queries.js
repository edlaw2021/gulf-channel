...
export async function getEventsForWeek(startDate, townId) {
  const { rows } = await query(
    `SELECT e.id, e.venue_id, e.performer_id, e.date, e.start_time, e.end_time, e.act_type, e.source, e.verified_at,
...
export async function getEventsForVenue(venueId) {
  const { rows } = await query(
    `SELECT e.id, e.date, e.start_time, e.end_time, e.act_type,
